...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    function setUp(canvas_pass, style = "#FFFFFF") {
        ...
        window.addEventListener('pointerdown', e => {
            ...
            if(walkmin.body.isPointInside(TIP_engine)){
                walkmin.body.anchored = 1
                walkmin.body.anchor = TIP_engine
            }else{

                for(let t = 0;t<walkmin.nodes.length;t++){
                    let link = new LineOP(TIP_engine, walkmin.nodes[t])
                    walkmin.nodes[t].xmom -= (link.hypotenuse()*Math.cos(link.angle()))/10
                    walkmin.nodes[t].ymom -= (link.hypotenuse()*Math.sin(link.angle()))/10
                    link.color = `rgb(${link.hypotenuse()/2}, ${255-(link.hypotenuse()/2)}, ${Math.random()*255})`
                    link.draw()
                }
            }
        ...
        window.addEventListener('pointerup', e => {
            ...
            // if(walkmin.body.isPointInside(TIP_engine)){
                walkmin.body.anchored = 0
                walkmin.body.xmom -=   (TIP_engine.lastx -  TIP_engine.x)
                walkmin.body.ymom -=   (TIP_engine.lasty -  TIP_engine.y)
            // }
        })
        function continued_stimuli(e) {
            ...
            TIP_engine.y = YS_engine
            TIP_engine.body = TIP_engine
            let link = new Line(TIP_engine.x, TIP_engine.y, TIP_engine.lastx, TIP_engine.lasty)

            if(walkmin.body.isPointInside(TIP_engine)){
                walkmin.body.anchored = 1
                walkmin.body.anchor = TIP_engine
                for(let t = 0;t<walkmin.nodes.length;t++){
                    walkmin.nodes[t].ang = (((walkmin.nodes[t].angstr+link.angle()+(Math.PI*.5))) + (walkmin.nodes[t].ang*16))/17
                    // if(link.angle()+Math.PI >  walkmin.nodes[t].ang){
                    //     walkmin.nodes[t].ang-= ( walkmin.nodes[t].ang-(link.angle()+Math.PI) ) /5 // / Math.sqrt(walkmin.nodes[t].dis+10)
                    // }else{
                    //     walkmin.nodes[t].ang += ((link.angle()+Math.PI )- walkmin.nodes[t].ang)   /5 // Math.sqrt(walkmin.nodes[t].dis+10)
                    // }
                    // if(walkmin.nodes[t].ang > Math.PI*2){
                    //     // walkmin.nodes[t].ang-= Math.PI*2
                    // }
                    // if(walkmin.nodes[t].ang < 0){
                    //     // walkmin.nodes[t].ang+= Math.PI*2
                    // }
                }
            }
    ...
    // canvas_context.translate(150,150)
    ...
    class Walkman {
        constructor(x,y, size, colors){
            this.gravity = .1
            this.body = new Circle(x,y, size, colors[0])
            this.body.reflect =1
            this.body.friction = .99
            this.head = new Circle(x,y-size*1.9, size*.9, "red")
            this.n1 = new Circle(x,y-size*1.5, size*.3, colors[0])
            this.n2 = new Circle(x,y-size*1, size*.3, colors[1])
            this.n3 = new Circle(x,y-size*.5, size*.3, colors[0])
            this.nodes = [this.body]
            for(let t = 0;t<9;t++){
                let torso = new Circle(this.body.x, this.body.y +( t*(size*(.2+(t/60)))), size-((size/6)*(t/6)), colors[t%2])
                this.nodes.push(torso)
            }
            for(let t = 0;t<7;t++){
                let arm = new Circle(this.body.x+(size+(size/6))  +( (t/6)*size), this.body.y +( t*(size*(.2+(t/60)))), (size-((size/6)*(t/6)))*.5, colors[((t+1)%2)])
                this.nodes.push(arm)
            }
            for(let t = 0;t<7;t++){
                let arm = new Circle((this.body.x-(size+(size/6)))  -( (t/6)*size), this.body.y +( t*(size*(.2+(t/60)))), (size-((size/6)*(t/6)))*.5, colors[((t+1)%2)])
            ...
            let lheight = (9*(size*(.2+(9/60))))
            for(let t = 0;t<10;t++){
                let arm = new Circle(this.body.x+(size*.2)+( (t/6)*size), this.body.y +( t*(size*(.2+(t/60)))) + lheight, (size-((size/6)*(t/6)))*.5, colors[((t+1)%2)])
                this.nodes.push(arm)
                let arm2 = new Circle(this.body.x-(size*.2) -( (t/6)*size), this.body.y +( t*(size*(.2+(t/60))))+ lheight, (size-((size/6)*(t/6)))*.5, colors[((t+1)%2)])
                this.nodes.push(arm2)
            }
            this.nodes.push(this.n1)
            this.nodes.push(this.n2)
            this.nodes.push(this.n3)
            this.nodes.push(this.head)
            this.wet = 0
            for(let t = 1;t<this.nodes.length;t++){
                let link = new LineOP(this.nodes[t], this.nodes[0], "red", 1)
                this.nodes[t].dis = link.hypotenuse()
                this.nodes[t].ang = link.angle()+Math.PI
                this.nodes[t].angstr = link.angle()+Math.PI
                this.nodes[t].friction = .95
                this.nodes[t].reflect = 1
            }
        }
        draw(){
            control(this.body, 3)
            ...
            this.wet = 0
            for(let t = 1;t<this.nodes.length;t++){
                // this.nodes[t].xmom += (Math.random()-.5)/10
                // this.nodes[t].ymom += (Math.random()-.5)/10
                if(this.body.anchored != 1){
                    this.nodes[t].ang =     ((this.nodes[t].ang*7)+   this.nodes[t].angstr)/8
                }
                this.nodes[t].xmom -= this.body.xmom/100
                this.nodes[t].ymom  -= this.body.ymom/100
                this.nodes[t].xmom -=   (this.nodes[t].x- ((((this.nodes[t].x*this.nodes[t].dis) + ((((Math.cos(this.nodes[t].ang-Math.PI)*this.nodes[t].dis))+(this.body.x)))*10))/(this.nodes[t].dis+10)))/(3+Math.random())
                this.nodes[t].ymom -=   (this.nodes[t].y- ((((this.nodes[t].y*this.nodes[t].dis) + ((((Math.sin(this.nodes[t].ang-Math.PI)*this.nodes[t].dis))+(this.body.y)))*10))/(this.nodes[t].dis+10)))/(3+Math.random())
                this.nodes[t].x = (((this.nodes[t].x*this.nodes[t].dis) + ((((Math.cos(this.nodes[t].ang-Math.PI)*this.nodes[t].dis))+(this.body.x)))*10))/(this.nodes[t].dis+10)
                this.nodes[t].y = (((this.nodes[t].y*this.nodes[t].dis) + ((((Math.sin(this.nodes[t].ang-Math.PI)*this.nodes[t].dis))+(this.body.y)))*10))/(this.nodes[t].dis+10)
                this.nodes[t].frictiveMove()
                if(this.nodes[t].x+this.nodes[t].radius > canvas.width){
                    this.nodes[t].x = canvas.width-this.nodes[t].radius
                }
                if(this.nodes[t].y+this.nodes[t].radius > canvas.height){
                    this.nodes[t].y = canvas.height-this.nodes[t].radius
                    this.wet = 1
                }
                if(this.nodes[t].x-this.nodes[t].radius  < 0){
                    this.nodes[t].x = 0+this.nodes[t].radius
                }
                if(this.nodes[t].y-this.nodes[t].radius< 0){
                    this.nodes[t].y = 0+this.nodes[t].radius
                }
                this.nodes[t].draw()
            }
            if(this.wet == 0){
    ...
    let walkmin = new Walkman(350,350, 22, ["red", "black"])
    function main() {
